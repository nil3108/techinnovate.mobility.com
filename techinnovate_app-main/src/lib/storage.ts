import type { Owner, Driver, Vehicle, Fill, Alert, AuditLog, Notification, CreditAction, PaymentEntry, CreditRequest } from './types'
...
const KEYS = {
  ...
  CREDIT_REQUESTS: 'cng_credit_requests',
...
function initDemoData() {
    ...
    const drivers: Driver[] = [
      {
        ...
        name: 'Amit Kumar',
        code: '1234',
        assignedVehicleId: 'GJ-01-AB-1234', // Store plate, not vehicle ID
        ownerId: 'own1',
        status: 'active',
        createdAt: new Date().toISOString(),
      },
      {
        ...
        name: 'Suresh Singh',
        code: '5678',
        assignedVehicleId: 'GJ-05-XY-5678', // Store plate, not vehicle ID
        ownerId: 'own1',
        status: 'active',
        createdAt: new Date().toISOString(),
      },
  ...
  if (!localStorage.getItem(KEYS.FILLS)) {
    const fills: Fill[] = [
      {
        id: 'fill1',
        vehicleId: 'GJ-01-AB-1234', // Use PLATE
        driverId: 'Amit Kumar', // Use NAME
...
export const storage = {
  ...
  getCreditRequests: (): CreditRequest[] => safeParse(localStorage.getItem(KEYS.CREDIT_REQUESTS), []),
  saveCreditRequests: (requests: CreditRequest[]) => localStorage.setItem(KEYS.CREDIT_REQUESTS, JSON.stringify(requests)),
  addCreditRequest: (req: CreditRequest) => {
    const reqs = safeParse<CreditRequest[]>(localStorage.getItem(KEYS.CREDIT_REQUESTS), [])
    reqs.push(req)
    localStorage.setItem(KEYS.CREDIT_REQUESTS, JSON.stringify(reqs))
  },
  updateCreditRequest: (id: string, updates: Partial<CreditRequest>) => {
    const reqs = safeParse<CreditRequest[]>(localStorage.getItem(KEYS.CREDIT_REQUESTS), [])
